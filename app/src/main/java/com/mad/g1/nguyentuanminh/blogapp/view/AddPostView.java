...
public class AddPostView extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == PICK_IMAGE_REQUEST && resultCode == RESULT_OK && data != null && data.getData() != null) {
            ...
            try {
                ...
                // Set the height of ImageView based on whether an image is selected or not
    ...
    private void postImage() {
        ...
        if (selectedImageUri != null && resizedBitmap != null) {
            // Create a Post object with other details
            Post post = new Post( title, content);

            // Post the image and other details using the ViewModel
            addPostViewModel.addPost(post, selectedImageUri);

        }
        else{
            Post post = new Post( title, content);
            addPostViewModel.addPost(post,null);
        }
